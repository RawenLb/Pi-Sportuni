...
public class DisciplineService implements ICourService<Discipline> {
    ...
    public List<Discipline> getAllDisciplines() {
        ...
        String query = "SELECT id_discipline, nom_discipline FROM discipline";
        try {
            ...
            while (rs.next()) {
                disciplines.add(new Discipline(
                        rs.getInt("id_discipline"),
                        rs.getString("nom_discipline"),
                        null, // Assuming description and image_url are not needed here
                        null
                ));
            }
        } catch (SQLException e) {
    ...
    public String getDisciplineNameById(int disciplineId) {
        String query = "SELECT nom_discipline FROM discipline WHERE id_discipline = ?";
        try {
            PreparedStatement ps = connexion.prepareStatement(query);
            ps.setInt(1, disciplineId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getString("nom_discipline");
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public Discipline getDisciplineById(int disciplineId) {
        ...
        String query = "SELECT * FROM discipline WHERE id_discipline = ?";
        try {
            PreparedStatement ps = connexion.prepareStatement(query);
            ps.setInt(1, disciplineId);
            ResultSet rs = ps.executeQuery();
