...
public class CourUserInterfaceController {
    ...
    private void inscrireAuCours(Cour cour) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            // Passer à l'interface historique des cours
            showNotifications(); // Appel de la méthode pour afficher les notifications
        }
    }


    public void handleButtonAction(ActionEvent event) {
        Image img = new javafx.scene.image.Image("C:\\Users\\ilyes\\d-lab\\PiDev-SporTuni-3A15\\src\\main\\Image\\2705.png");
        ...
        Notifications notificationBuilder = Notifications.create()
                ...
                .onAction(new EventHandler<ActionEvent >() {
                    ...
                    public void handle(ActionEvent event) {
                        // Afficher l'interface de notification lorsqu'on clique sur la notification
                        showNotifications();
    ...
    private void saveNotification(Cour cour) {
        ...
        String query = "INSERT INTO otifications (nom_cour, nom_discipline, date, heure_debut, nom_salle) VALUES (?, ?, ?, ?, ?)";
        try {
            PreparedStatement statement = conn.prepareStatement(query);
            statement.setString(1, cour.getNom_cour());
            statement.setString(2, cour.getDiscipline().getNom_discipline());
    ...
    private String formatCourseDetails(Cour cour) {
        return String.format("Nom du cours : %s\nDiscipline : %s\nDate : %s\nHeure début : %s\nNom de la salle : %s\n",
                cour.getNom_cour(),cour.getDiscipline(), cour.getDate(), cour.getHeure_debut(), cour.getNom_salle());
    }
