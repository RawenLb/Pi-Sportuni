...
public class CourDashbordController  {
    ...
    @FXML
    private ComboBox<Integer> tfdesc2;
    ...
    void initialize() {
        try {
            // Initialisation du TableView avec les données des cours
            ObservableList<Cour> observableList = FXCollections.observableList(cs.readAll());
            tvCour.setItems(observableList);

            // Configuration des cellules du TableView pour afficher les différentes propriétés des cours
            clNom.setCellValueFactory(new PropertyValueFactory<>("nom_cour"));
            clDiscipline.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getDiscipline().getNom_discipline()));
            ...
            // Initialisation du service de discipline
            ...
            ObservableList<Integer> disciplineIds = FXCollections.observableArrayList();
            for (Discipline discipline : disciplines) {
                disciplineIds.add(discipline.getId_discipline());
            }

            // Configuration du ComboBox pour afficher les IDs des disciplines
            tfdesc2.setItems(disciplineIds);

            // Ajout d'un listener pour mettre à jour les autres champs lorsque la sélection change
            tvCour.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
                if (newValue != null) {
                    cour = newValue;
                    tfnom2.setText(newValue.getNom_cour());
                    tfdesc2.setValue(newValue.getDiscipline().getId_discipline());
                    tfdate.setValue(newValue.getDate().toLocalDate());
                    tfhd.setText(newValue.getHeure_debut());
                    tfhf.setText(newValue.getHeure_fin());
                    tfns.setText(newValue.getNom_salle());
                    tfnbm.setText(newValue.getNb_max());
                }
            });
    ...
    void handleGeneratePDF(ActionEvent actionEvent) {
        try {
            ...
            for (Cour cour : cours) {
                ...
                row.createCell(1).setCellValue(cour.getDiscipline().getNom_discipline());
    ...
    void add2(ActionEvent event) {
        try {
            ...
            Integer disciplineId = tfdesc2.getValue(); // Récupérer l'ID de la discipline sélectionnée
            ...
            if (nomCours.isEmpty() || disciplineId == null || heureDebut.isEmpty() || heureFin.isEmpty() || nomSalle.isEmpty() || nb_max.isEmpty()) {
            ...
            // Récupérer l'objet Discipline correspondant à l'ID de discipline sélectionné
            Discipline selectedDiscipline = disciplineService.getDisciplineById(disciplineId);

            // Créer un nouvel objet Cour avec les données saisies
            Cour nouveauCour = new Cour(nomCours, Date.valueOf(localDate), heureDebut, heureFin, nomSalle, nb_max, selectedDiscipline);
    ...
    void mouseClicked2(MouseEvent mouseEvent) {
        try {
            ...
            if (s != null) {
                ...
                tfdesc2.getSelectionModel().select(cour.getDiscipline().getId_discipline());
            ...
            searchField.textProperty().addListener((observable, oldValue, newValue) -> {
                filteredList.setPredicate(cour -> {
                    ...
                    return cour.getNom_cour().toLowerCase().contains(lowerCaseFilter)
                            || cour.getDiscipline().getNom_discipline().toLowerCase().contains(lowerCaseFilter)
    ...
    void update2(ActionEvent event) {
        try {
            if (cour != null) {
                // Récupérer les valeurs des champs depuis l'interface utilisateur
                String nomCours = tfnom2.getText().trim();
                Integer disciplineId = tfdesc2.getValue();
                String heureDebut = tfhd.getText().trim();
                String heureFin = tfhf.getText().trim();
                String nomSalle = tfns.getText().trim();
                String nb_max = tfnbm.getText().trim();
                ...
                cour.setDiscipline(disciplineService.getDisciplineById(disciplineId));
    ...
    // Dans la classe CourDashbordController
    public void statArea() {
        try {
            ObservableList<Cour> courList = FXCollections.observableList(cs.readAll());

            // Initialisation d'une map pour stocker le nombre de cours par discipline
            Map<String, Long> disciplineCounts = new HashMap<>();

            // Comptage du nombre de cours par discipline
            for (Cour cour : courList) {
                String disciplineName = cour.getDiscipline().getNom_discipline();
                disciplineCounts.put(disciplineName, disciplineCounts.getOrDefault(disciplineName, 0L) + 1);
            }

            // Effacement des données précédentes du graphique
            ...
            for (Map.Entry<String, Long> entry : disciplineCounts.entrySet()) {
    ...
    @FXML
    private Button btn_discipline1;
