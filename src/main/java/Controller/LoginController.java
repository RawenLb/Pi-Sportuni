...
public class LoginController {
    ...
    void login(ActionEvent event) throws IOException {
        if(tfemail.getText().equals("admin@admin.com") && tf_pwd.getText().equals("adminadmin") )
        {
            ...
            try{
                ...
                byte[] hash = md.digest(tf_pwd.getText().getBytes());
                ...
                smt.setString(2,HASHED_MDP);
                ResultSet rs= smt.executeQuery();
                User p;
                if(rs.next()){
                    p=new User( rs.getInt("id"), rs.getString("nom"), rs.getString("prenom"), rs.getString("email"), rs.getString("password"), rs.getInt("numero"), rs.getString("adress"));
